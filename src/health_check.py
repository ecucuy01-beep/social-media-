...
def check_discord():
          ...
          try:
                        ...
                        if response.status_code == 200:
                                          print(f"SUCCESS: Discord Bot Token is valid (User: {response.json()['username']})")
          else:
                            print(f"ERROR: Discord API Error: {response.status_code} - {response.text}")
except Exception as e:
...
def check_meta():
          ...
          try:
                        ...
                        if response.status_code == 200:
                                          print(f"SUCCESS: Meta Access Token is valid (User ID: {response.json().get('id')})")
          else:
                            print(f"ERROR: Meta API Error: {response.status_code} - {response.text}")
except Exception as e:
        print(f"ERROR: Meta Error: {e}")
