...
def parse_message_content(content):
          """Extracts caption and client_id from Discord message content."""
          # Heuristic: Client ID is usually in backticks at the end
          match = re.search(r"Client ID: `(.+?)`", content)
          client_id = match.group(1) if match else None
          # Caption is everything before "Client ID:"
    ...
    # Clean up the caption a bit more specifically
          if "**DRAFT POST FOR" in content:
                        lines = content.split("\n")
                        # Find the first line that isn't the header and isn't empty
    ...
    # Companies lookup for IDs
    companies_map = {c["client_id"]: c for c in config["companies"]}

    # Poll messages from Discord channels
    # We'll collect all distinct channel IDs from the config
    channel_ids = set()
    for company in config["companies"]:
                  if "discord_channel_id" in company:
                                    channel_ids.add(company["discord_channel_id"])
                                # If it was an env var placeholder in config:
    ...
    for channel_id in channel_ids:
                ...
                if reactions:
                    ...
                    if not client_id or client_id not in companies_map:
                                                  print(f"Warning: Could not identify client_id '{client_id}' in message {msg_id}")
                                                  continue
                    ...
                    # Post to Facebook
                    if fb_page_id:
                                                  print(f"Posting to Facebook for {client_id}...")
                                                  meta.post_to_facebook(fb_page_id, caption, image_url)

                    # Post to Instagram
                    if ig_account_id and image_url:
                                                  print(f"Posting to Instagram for {client_id}...")
                                                  meta.post_to_instagram(ig_account_id, caption, image_url)

                    # Delete message from Discord once posted
                    print(f"Deleting message {msg_id} from Discord...")
                    try:
                                                  discord.delete_message(channel_id, msg_id)
except Exception as de:
                        print(f"Warning: Failed to delete message {msg_id}: {de}")
else:
                    print(f"Message {msg_id} not approved yet or no reactions found.")
